...
void common_signal_procedures (PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc) {
  ...
  printf("[common_sigial_procedures] zh frame=%d,subframe=%d\n",frame,subframe);
  fflush(stdout);
  ...
  if (subframe == 0) {
    ...
    if (eNB->abstraction_flag==0) {
      generate_pbch(&eNB->pbch,
                    ...
                    pbch_pdu,
                    frame&3);
	  // zh add mypbch 20171025
	struct ENB_DL_TYPE1_PBCH_C pbchc;
	memset(&pbchc,0,sizeof(ENB_DL_TYPE1_PBCH_C));
	pbchc.NumPrbBw=htons(fp->N_RB_DL);
	pbchc.PhichDurtion=htons(fp->phich_config_common.phich_duration);
	pbchc.GPhichNg=htons(fp->phich_config_common.phich_resource);
	pbchc.spare=htons(0);
	struct ENB_DL_TYPE1_PBCH_D pbchd;
	memcpy(&pbchd.SourcePBCH,eNB->pbch_pdu,4*sizeof(uint8_t));
	printf("[common_signal_procedures]zh get pbch N_RB_DL=%d,duration=%d,resource=%d\n",fp->N_RB_DL,fp->phich_config_common.phich_duration,fp->phich_config_common.phich_resource);
	fflush(stdout);
	int len=sizeof(struct ENBPHYADPtoPHYType1);
	memcpy((void*)eNB->common_vars.tx_dspdata[0]+len,&pbchc,sizeof(struct ENB_DL_TYPE1_PBCH_C));
	len+=sizeof(struct ENB_DL_TYPE1_PBCH_C);
	memcpy((void*)eNB->common_vars.tx_dspdata[0]+len,&pbchd,sizeof(struct ENB_DL_TYPE1_PBCH_D));
	eNB->common_vars.dsp_data_flag=1;
	printf("[common_signal_procedures]pbch end offset=%d\n",len);
	fflush(stdout);
	// zh end
    }
#ifdef PHY_ABSTRACTION
...
void generate_eNB_dlsch_params(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc,DCI_ALLOC_t *dci_alloc,const int UE_id) {
  printf("[generate_eNB_dlsch_params]zh enter\n");
  LTE_DL_FRAME_PARMS *fp=&eNB->frame_parms;
  int frame = proc->frame_tx;
  int subframe = proc->subframe_tx;
  ...
  if (dci_alloc->rnti == SI_RNTI) {
	...
	//printf("[generate_eNB_dlsch_params] Generating dlsca params for SI_RNTI\n");
	...
	printf("Generating dlsch params for RA_RNTI\n");
    generate_eNB_dlsch_params_from_dci(frame,
				       subframe,
				       &dci_alloc->dci_pdu[0],
				       dci_alloc->rnti,
...
uint16_t pdsch_procedures(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc,LTE_eNB_DLSCH_t *dlsch, LTE_eNB_DLSCH_t *dlsch1,LTE_eNB_UE_stats *ue_stats,int ra_flag,int num_pdcch_symbols,uint16_t dsp_offset) {
  ...
  if (dlsch_harq->round == 0) {
	...
	DLSCH_pdu = mac_xface->get_dlsch_sdu(eNB->Mod_id,
					     ...
					     dlsch->rnti,
					     0);
	//zh add 20171025 process our dsp data
	//test zh 
	/*
	printf("[pdsch_procesures]zh DLSCH_pdu=\n");
	for(int i=0;i<128;i++){
	...
	//memset(&pdschc,0,sizeof(pdschc));
	//pdschc.RNTI=htons(SI_RNTI);
	...
	dsp_offset+=SI_LENGTH; 
			...
			// ue special data
	//	struct ENB_DL_TYPE1_PDSCH_C pdschc;
	//	memset(&pdschc,0,sizeof(ENB_DL_TYPE1_PDSCH_C));
	//	pdschc.RNTI=htons(dlsch->rnti);
	...
	//	printf("[pdsch_procedures] begine ue data  dsp_offset=%d,pdschd=%d\n",dsp_offset,sizeof(ENB_DL_TYPE1_PDSCH_D));
		...
		uint16_t datalen=dlsch->harq_processes[dlsch->current_harq_pid]->TBS>>3;
		pdschd.PdschData=htons(datalen);
		memcpy((void*)eNB->common_vars.tx_dspdata[0]+dsp_offset,&pdschd,sizeof(ENB_DL_TYPE1_PDSCH_D));
		dsp_offset+=sizeof(ENB_DL_TYPE1_PDSCH_D);
	...
	if (UE_id==-1) {
	  ...
	  mac_xface->cancel_ra_proc(eNB->Mod_id,
				    ...
				    frame,
				    crnti);
	 printf("[pdsch_procedures] cancel_ra_proc\n");
	 fflush(stdout);
	} else {
	 //zh add 20171026 init ra
     struct OPENAIR_TO_SRIO_DL *header=(struct OPENAIR_TO_SRIO_DL*)eNB->common_vars.tx_buff[0];
	 uint16_t len=sizeof(struct OPENAIR_TO_SRIO_DL);
	 header->msg2_offset=len;
	 header->dsp_offset=len+input_buffer_length;
	 memcpy((void*)eNB->common_vars.tx_buff[0]+len,DLSCH_pdu,input_buffer_length);
	 /*
	 for(int i=0;i<10;i++){
			 printf("%d %x ",i,DLSCH_pdu_rar[i]);
	 }*/
	 printf("[pdsch_procedures] msg2 len=%d\n",input_buffer_length);
	 fflush(stdout);
	 //zh end
	  eNB->UE_stats[(uint32_t)UE_id].mode = RA_RESPONSE;
	  printf("[pdsch_procedures]zh set ue_id=%d to ra_response\n",(uint32_t)UE_id);
			   ...
			   int do_meas)
{
  ...
  if (is_pmch_subframe(frame,subframe,fp)) {
    ...
    //printf("[phy_procedures_eNB_TX] zh will enter commong_signal_procesures\n");
	//flush(stdout);
  ...
  if (eNB->abstraction_flag == 0) {
	...
	printf("[phy_procedures_eNB_TX] num_common_dci=%d,num_ue_speci_dci=%d\n",DCI_pdu->Num_common_dci,DCI_pdu->Num_ue_spec_dci);
	fflush(stdout);
    if (DCI_pdu->Num_ue_spec_dci+DCI_pdu->Num_common_dci > 0) {
      LOG_D(PHY,"[eNB %"PRIu8"] Frame %d, subframe %d: Calling generate_dci_top (pdcch) (common %"PRIu8",ue_spec %"PRIu8")\n",eNB->Mod_id,frame, subframe,
            DCI_pdu->Num_common_dci,DCI_pdu->Num_ue_spec_dci);
	//2017111 在自己的pdcch添加dci 假设只有一个si_dci 和一个ra_dci，可能会有问题
	int j=2;
	int public_offset=sizeof(ENBPHYADPtoPHYType1)+sizeof(ENB_DL_TYPE1_PBCH_C)+sizeof(ENB_DL_TYPE1_PBCH_D)+2*sizeof(ENB_DL_TYPE1_PHICH_C)+2*sizeof(ENB_DL_TYPE1_PHICH_D);
	ENB_DL_TYPE1_PDCCH_C *pdcchc=(ENB_DL_TYPE1_PDCCH_C*)((void*)eNB->common_vars.tx_dspdata[0]+public_offset);
	ENB_DL_TYPE1_PDCCH_D *pdcchd=(ENB_DL_TYPE1_PDCCH_D*)((void*)eNB->common_vars.tx_dspdata[0]+public_offset+sizeof(ENB_DL_TYPE1_PDCCH_C)*5);
	...
	for (i=0; i<DCI_pdu->Num_common_dci + DCI_pdu->Num_ue_spec_dci ; i++) {  
		...
		if(dci_alloc->rnti==SI_RNTI){
				tpdcchc=pdcchc;
				tpdcchd=pdcchd;
		}else if(dci_alloc->ra_flag==1){
				tpdcchc=(ENB_DL_TYPE1_PDCCH_C*)((void*)pdcchc+lenc);
				tpdcchd=(ENB_DL_TYPE1_PDCCH_D*)((void*)pdcchd+lend);
				eNB->common_vars.dsp_data_flag=1;
		}else{
				tpdcchc=(ENB_DL_TYPE1_PDCCH_C*)((void*)pdcchc+lenc*j);		
				tpdcchd=(ENB_DL_TYPE1_PDCCH_D*)((void*)pdcchd+lend*j);
				j++;
		}
		tpdcchc->RNTI=htons(dci_alloc->rnti);
		tpdcchc->PdcchFormat=(uint8_t)dci_alloc->format;
		tpdcchc->CommonPdcchFlag=htons(dci_alloc->ra_flag);
		tpdcchc->DciBitLen=htons(dci_alloc->dci_length);
		tpdcchd->RNTI=htons(dci_alloc->rnti);
		printf("[phy_procedures_eNB_TX] dci_rnti=%d,dci_format=%d,ra_flag=%d,dci_len=%d\n",dci_alloc->rnti,(uint8_t)dci_alloc->format,dci_alloc->ra_flag,dci_alloc->dci_length);
		memcpy(&(tpdcchd->DataSource),dci_alloc->dci_pdu,sizeof(tpdcchd->DataSource));
		/*
		printf("[phy_procedures_eNB_TX] datasource=%llx\n",tpdcchd->DataSource);
		printf("[phy_procedures_eNB_TX] dci_alloc=%x %x %x %x %x %x %x %x\n",dci_alloc->dci_pdu[0],dci_alloc->dci_pdu[1],dci_alloc->dci_pdu[2],dci_alloc->dci_pdu[3],
						dci_alloc->dci_pdu[4],dci_alloc->dci_pdu[5],dci_alloc->dci_pdu[6],dci_alloc->dci_pdu[7]);*/
		//printf("[phy_procedures_eNB_TX] si pdcchd offset=%d,pdcchc offset=%d\n",public_offset+sizeof(ENB_DL_TYPE1_PDCCH_C)*5,public_offset);
		}
	}
  ...
  uint16_t dsp_offset=0;
    //zh add init dsp_offset 20171026
  dsp_offset=sizeof(struct ENBPHYADPtoPHYType1) +
			sizeof(struct ENB_DL_TYPE1_PBCH_C) +
			sizeof(struct ENB_DL_TYPE1_PBCH_D) +
			2 * sizeof(struct ENB_DL_TYPE1_PHICH_C) +
			2 * sizeof(struct ENB_DL_TYPE1_PHICH_D) +
			5 * sizeof(struct ENB_DL_TYPE1_PDCCH_C) +
			5 * sizeof(struct ENB_DL_TYPE1_PDCCH_D)+
			5 * sizeof(struct ENB_DL_TYPE1_PDSCH_C);
  printf("[phy_procedures_eNB_TX]zh init pdschd_offset=%d\n",dsp_offset);
  ...
  if ((eNB->dlsch_ra) && (eNB->dlsch_ra->active == 1)) {
    ...
    LOG_I(PHY,"[eNB %"PRIu8"][RAPROC] Frame %d, subframe %d: Calling generate_dlsch (RA),Msg3 frame %"PRIu32", Msg3 subframe %"PRIu8"\n",
	  eNB->Mod_id,
	  frame, subframe,
	  eNB->ulsch[(uint32_t)UE_id]->Msg3_frame,
	  eNB->ulsch[(uint32_t)UE_id]->Msg3_subframe);
    
    pdsch_procedures(eNB,proc,eNB->dlsch_ra,(LTE_eNB_DLSCH_t*)NULL,(LTE_eNB_UE_stats*)NULL,1,num_pdcch_symbols,0);
  ...
  */
// zh add 20171117 在头部中填写帧号和子帧号
struct ENBPHYADPtoPHYType1 *dsp_header=(ENBPHYADPtoPHYType1*)eNB->common_vars.tx_dspdata[0];
struct GeneralHeader12 *general_header=(GeneralHeader12*)(&(dsp_header->GHead));
general_header->SFN=htons(frame);
general_header->SubframeN=htons(subframe);
// zh add 20171026 combination pdu
struct OPENAIR_TO_SRIO_DL *header=(struct OPENAIR_TO_SRIO_DL*)eNB->common_vars.tx_buff[0];
if(header->msg2_offset==0){
		header->dsp_offset=sizeof(OPENAIR_TO_SRIO_DL);
}
if(eNB->common_vars.dsp_data_flag!=0){
	header->dsp_length=dsp_offset;
	memcpy((void*)eNB->common_vars.tx_buff[0]+header->dsp_offset,eNB->common_vars.tx_dspdata[0],header->dsp_length);
//	printf("address eNB->common_vars.tx_buff[0]=%p\n",(void*)eNB->common_vars.tx_buff[0]);
//	printf("address eNB->common_vars.tx_buff[0]+header->dsp_offset=%p\n",(void*)eNB->common_vars.tx_buff[0]+header->dsp_offset);
}
printf("[phy_procedures_eNB_TX] zh msg2_offset=%d,dsp_offset=%d,dsp_datalen=%d\n",header->msg2_offset,header->dsp_offset,header->dsp_length);
fflush(stdout);
//转换成网络字节序
header->msg2_offset=htons(header->msg2_offset);
...
void process_Msg3(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc,uint8_t UE_id, uint8_t harq_pid)
{
  ...
  printf("[process_Msg3] ulsch msg3_flag=%d,msg3_subframe=%d,frame=%d,current subframe=%d,frame=%d\n",eNB->ulsch[(uint32_t)UE_id]->Msg3_active,
				  eNB->ulsch[(uint32_t)UE_id]->Msg3_subframe,eNB->ulsch[(uint32_t)UE_id]->Msg3_frame,subframe,frame);
  //zh 20171225
  if ((eNB->ulsch[(uint32_t)UE_id]->Msg3_active == 1) &&
    ...
    //    harq_pid = 0;
   // printf("[process_Msg3] set flag\n");
   // OPENAIR_TO_SRIO_UP* header=(OPENAIR_TO_SRIO_UP*)eNB->common_vars.rx_buff[0];
   // if(header->rach_msg_flag==3){
    eNB->ulsch[(uint32_t)UE_id]->Msg3_active = 0;
    eNB->ulsch[(uint32_t)UE_id]->Msg3_flag = 1;
    eNB->ulsch[(uint32_t)UE_id]->harq_processes[harq_pid]->subframe_scheduling_flag=1;
    LOG_I(PHY,"[eNB %d][RAPROC-process_Msg3] frame %d, subframe %d: Setting subframe_scheduling_flag (Msg3) for UE %d\n",
          eNB->Mod_id,
          frame,subframe,UE_id);
  }
}
                           ...
                           uint8_t SR_payload)
{
  ...
  printf("[process_HARQ_feedback]enter,pusch_flag=%d\n",pusch_flag);
  LTE_DL_FRAME_PARMS *fp=&eNB->frame_parms;
...
void prach_procedures_zh(PHY_VARS_eNB *eNB) {
  ...
  uint8_t CC_id = eNB->CC_id;
    ...
    OPENAIR_TO_SRIO_UP* header=(OPENAIR_TO_SRIO_UP*)eNB->common_vars.rx_buff[0];
	if(header->rach_msg_flag==1){
	MSG1_Context* msg=(MSG1_Context*)((void*)eNB->common_vars.rx_buff[0]+sizeof(OPENAIR_TO_SRIO_UP));
	uint8_t preamble=msg->preamble_index;
	printf("[prach_procedures_zh] preamble=%d frame=%d,subframe=%d\n",preamble,frame,subframe);
	fflush(stdout);
	mac_xface->initiate_ra_proc(eNB->Mod_id,
				    eNB->CC_id,
				    frame,
...
#endif
// zh change 20180521
        /*process_HARQ_feedback_zh(UE_id,eNB,proc,
                            0,// pusch_flag
                            pucch_payload0,
                            2,
                            SR_payload);*/
...
void phy_procedures_eNB_common_RX(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc){
  ...
  printf("[phy_procedures_eNB_common_RX]zh enter\n");
  //  eNB_proc_t *proc       = &eNB->proc;
  LTE_DL_FRAME_PARMS *fp = &eNB->frame_parms;
  const int subframe     = proc->subframe_rx;
  const int frame        = proc->frame_rx;
...
void phy_procedures_eNB_uespec_RX(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc,const relaying_type_t r_type)
{
  ...
  OPENAIR_TO_SRIO_UP* test=(OPENAIR_TO_SRIO_UP*)eNB->common_vars.rx_buff[0];
   uint16_t test_length=ntohs(test->dsp_length);
  printf("[phy_procedures_eNB_uespec_RX] enter,dsp_length=%d\n",test_length);
  UNUSED(r_type);
  ...
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
    ...
    // check for Msg3
	//printf("[phy_procedures_eNB_uespec_RX] check msg3 mac_enable=%d,%d ue mode=%d\n",eNB->mac_enabled,i,eNB->UE_stats[i].mode);
    if (eNB->mac_enabled==1) {
...
//	if(eNB->ulsch[i])
//    	printf("[phy_procedures_eNB_uespec_RX] ulsch[%d] rnti=%d,subframe_scheduling_flag=%x\n",i,eNB->ulsch[i]->rnti,eNB->ulsch[i]->harq_processes[harq_pid]->subframe_scheduling_flag);
	...
	/*
	  ...
	  if(dsp_length>0){
		...
		UE_UL_TYPE1_PUSCH_D* puschd=(UE_UL_TYPE1_PUSCH_D*)((void*)eNB->common_vars.rx_buff[0]+dsp_offset);
		uint16_t rnti=ntohs(puschd->RNTI);
		printf("[phy_procedures_eNB_uespec_RX]zh rx_sdu,dsp_offset=%d,dsp_length=%d,rnti=%d\n",dsp_offset,dsp_length,rnti);
	    mac_xface->rx_sdu(eNB->Mod_id,
			      eNB->CC_id,
			      frame,subframe,
			      rnti,
			      (void*)eNB->common_vars.rx_buff[0]+dsp_offset+sizeof(UE_UL_TYPE1_PUSCH_D),
			      dsp_length,
			      harq_pid,
			      NULL);
	   }
	...
	*/
    ...
    // zh process msg3  20171222
	OPENAIR_TO_SRIO_UP* header=(OPENAIR_TO_SRIO_UP*)eNB->common_vars.rx_buff[0];
	printf("[phy_procedures_eNB_uespec_RX]ulsch=%d, rach flag=%d\n",i,header->rach_msg_flag);
	if(header->rach_msg_flag==3&&eNB->ulsch[i]->harq_processes[harq_pid]->subframe_scheduling_flag==1){
		uint8_t*  msg3=(uint8_t*)((void*)eNB->common_vars.rx_buff[0]+sizeof(OPENAIR_TO_SRIO_UP));
		uint16_t msg3_len=ntohs(header->dsp_offset)-sizeof(OPENAIR_TO_SRIO_UP);
		printf("[phy_procedures_eNB_uespec_RX] msg3_len=%d\n",msg3_len);
	/*	for(int m=0;m<msg3_len;m++)
				printf("%x ",msg3[m]); */
	 	if (eNB->mac_enabled)
	    	  mac_xface->rx_sdu(eNB->Mod_id,
					eNB->CC_id,
  ...
  //process ul data 20180109
	...
	eNB->rb_mask_ul[rb2>>5] |= (1<<(rb2&31));
      ...
      if (eNB->ulsch[i]->Msg3_flag == 1) {
        LOG_I(PHY,"[eNB %d] frame %d, subframe %d: Scheduling ULSCH Reception for Msg3 in Sector %d\n",
              eNB->Mod_id,
              frame,
              subframe,
	...
	VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_PHY_ENB_ULSCH_MSG3,1);
        ...
        LOG_I(PHY,"[eNB %d] frame %d, subframe %d: Scheduling ULSCH Reception for UE %d Mode %s\n",
              eNB->Mod_id,
              frame,
              subframe,
              i,
    ...
    /* zh process msg3  20171222
	OPENAIR_TO_SRIO_UP* header=(OPENAIR_TO_SRIO_UP*)eNB->common_vars.rx_buff[0];
	printf("[phy_procedures_eNB_uespec_RX]zh msg flag=%d\n",header->rach_msg_flag);
	if(header->rach_msg_flag==3){
		uint8_t*  msg3=(uint8_t*)((void*)eNB->common_vars.rx_buff[0]+sizeof(OPENAIR_TO_SRIO_UP));
		uint16_t msg3_len=ntohs(header->dsp_offset)-sizeof(OPENAIR_TO_SRIO_UP);
		printf("[phy_procedures_eNB_uespec_RX] msg3_len=%d\n",msg3_len);
