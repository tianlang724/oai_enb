                      ...
                      uint8_t f_id)
{
  ...
  for (i=0; i<NB_RA_PROC_MAX; i++) {
	printf("[initiate_ra_proc] zh ra_active=%d,wait_ack_msg4=%d\n",RA_template[i].RA_active,RA_template[i].wait_ack_Msg4);
	fflush(stdout);
    if (RA_template[i].RA_active==FALSE &&
	  ...
	  RA_template[i].RA_rnti= 4; //20171216
      RA_template[i].preamble_index = preamble_index;
	  printf("[initiate_ra_proc] ra_rnti=%x,preamble_index=%x\n",RA_template[i].RA_rnti,preamble_index);
