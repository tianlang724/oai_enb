...
void tx_fh_if5(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc) {
       ...
       (subframe_select(&eNB->frame_parms,proc->subframe_tx) != SF_UL)))    
  {
	...
	if(header->msg2_offset==0&&header->dsp_length==0){
			printf("[tx_fh_if5] nothing send\n");
			fflush(stdout);
			return ;
	}
	LTE_DL_FRAME_PARMS *fp = &eNB->frame_parms;
  	int siglen=fp->samples_per_tti,flags=1;
  	eNB->ifdevice.trx_write_func(&eNB->ifdevice,null,(void**)eNB->common_vars.tx_buff,siglen,fp->nb_antennas_tx,flags);
  }
}
...
void proc_tx_high0(PHY_VARS_eNB *eNB,
  ...
  //printf("[proc_tx_high0] zh return from phy_procedures_eNB_TX\n");
  //fflush(stdout);
...
static inline int rxtx(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc, char *thread_name) {
  ...
  printf("[rxtx]frame_rx=%d,subframe_rx=%d\n",proc->frame_rx,proc->subframe_rx);
  ...
  printf("[rxtx]zh msg flag=%d,dsp_offset=%d\n",header->rach_msg_flag,ntohs(header->dsp_offset));
...
static void* eNB_thread_single( void* param ) {
  ...
  while (!oai_exit) {
    ...
    // zh add for test
	//proc_rxtx->subframe_rx=(subframe+4)%10;
	proc_rxtx->subframe_rx=last_subframe;
	//proc_rxtx->subframe_rx=subframe;
	proc_rxtx->subframe_tx=subframe;
	//proc_rxtx->frame_rx=frame;
	proc_rxtx->frame_rx=last_frame;
	proc_rxtx->frame_tx=frame;
